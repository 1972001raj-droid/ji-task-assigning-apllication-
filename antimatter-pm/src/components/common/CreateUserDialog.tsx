...
export function CreateUserDialog({ open, onClose, onCreated }: Props) {
  ...
  const [role, setRole] = useState<'MANAGER' | 'DEVELOPER' | 'TESTER' | 'DEVELOPER_TESTER'>(isAdmin ? 'MANAGER' : 'DEVELOPER');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(typeof detail === 'string' ? detail : 'Failed to provision user');
    } finally {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                  {isAdmin ? 'Provision Team Account' : 'Provision Developer / Tester'}
                </h2>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {isAdmin ? 'Create a Manager, Developer, or Tester account' : 'Provision a new Developer or Tester account for your team'}
                </p>
              </div>
            </div>
            <button
